...
import iconnftzip from 'assets/svgs/logo_small_blue.svg';
...
const NFTItem = () => {
  ...
  const isLoggedIn = () => {
    return CHAIN[process.env.CHAIN_ID] ?? true;
  };
  ...
  useEffect(() => {
    ...
    getPrices();
    setPriceInterval(setInterval(getPrices, 1000 * 100));
  ...
  const getItemDetails = async () => {
    ...
    try {
      ...
      console.log(contentType, history, likes, listings);
  ...
  const handleRevealContent = async () => {
    ...
    try {
      ...
      try {
        ...
        const msg = `Approve Signature on nftzip.io with nonce ${nonce}`;
  ...
  const handleShareToTwitter = () => {
    handleClose();
    window.open(
      `https://twitter.com/intent/tweet?text=Check%20out%20this%20item%20on%20nftzip&url=${window.location.href}`,
      '_blank'
    );
  };
